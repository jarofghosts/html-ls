...
function lsHtml(dir, _options) {
  ...
  options.transformHref = options.transformHref || identity
  options.transformLinkText = options.transformLinkText || identity
    ...
    function done() {
      ...
      function streamItem(objectName) {
        var objectString = util.format(
          '<%s><a href="%s">%s</a></%s>\n',
          options.childTag,
          options.transformHref(objectName),
          options.transformLinkText(objectName),
          options.childTag
        )
